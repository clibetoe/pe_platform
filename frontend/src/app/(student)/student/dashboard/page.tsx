...
/* Reusable bubble decoration for white cards */
function CardBubbles({ a = "brand", b = "accent" }: { a?: string; b?: string }) {
  const colors: Record<string, string> = {
    brand: "bg-brand-400/[0.06]", accent: "bg-accent-400/[0.07]",
    violet: "bg-violet-400/[0.05]", emerald: "bg-emerald-400/[0.06]",
    amber: "bg-amber-400/[0.06]", rose: "bg-rose-400/[0.05]",
  };
  return (
    <>
      <div className={`pointer-events-none absolute -top-8 -right-8 w-36 h-36 rounded-full ${colors[a]}`} />
      <div className={`pointer-events-none absolute -bottom-6 -left-6 w-28 h-28 rounded-full ${colors[b]}`} />
      <div className="pointer-events-none absolute top-8 right-1/3 w-10 h-10 rounded-full bg-violet-400/[0.04]" />
    </>
  );
}
...
export default function StudentDashboard() {
  ...
  const statCards = stats
    ? [
        { label: "Lessons Done",    value: `${stats.lessons_completed}`, sub: `of ${stats.total_lessons}`, icon: BookOpen,      gradient: "from-brand-500 to-brand-700",      bg: "from-brand-50 to-indigo-50",    ring: "ring-brand-100" },
        { label: "Quizzes Passed",  value: `${stats.quizzes_passed}`,    sub: "total",                     icon: ClipboardCheck, gradient: "from-emerald-500 to-teal-600",    bg: "from-emerald-50 to-teal-50",    ring: "ring-emerald-100" },
        { label: "Avg. Score",      value: formatScore(stats.avg_score), sub: "overall",                   icon: TrendingUp,     gradient: "from-amber-400 to-orange-500",    bg: "from-amber-50 to-orange-50",    ring: "ring-amber-100" },
        { label: "XP Earned",       value: `${stats.xp_total}`,          sub: "points",                    icon: Zap,            gradient: "from-violet-500 to-purple-600",   bg: "from-violet-50 to-purple-50",   ring: "ring-violet-100" },
      ]
    : [];
          ...
          {stats && (
            <div className="relative z-10 mt-6">
              ...
              <div className="h-2 rounded-full bg-white/15 overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-accent-400 to-brand-300 transition-all duration-700" style={{ width: `${progressPct}%` }} />
              </div>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {statCards.map(({ label, value, sub, icon: Icon, gradient, bg, ring }) => (
              <div key={label} className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${bg} border border-white ring-1 ${ring} p-5 group hover:-translate-y-0.5 transition-all duration-200`}>
                {/* White bubble decorations on colored bg */}
                  ...
                  recentAttempts.map((a) => (
                    <div key={a.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-gray-50/60 transition-colors">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                        <span className={`text-sm font-bold ${a.passed ? "text-emerald-600" : "text-red-500"}`}>{formatScore(a.score)}</span>
                ...
                {certs.length === 0 ? (
                  ...
                  certs.map((c) => (
                    <div key={c.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50/60 transition-colors">
                      ...
                      <span className="text-xs font-semibold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-full flex-shrink-0">Certified</span>
                    </div>
                  ))
                )}
              </div>
